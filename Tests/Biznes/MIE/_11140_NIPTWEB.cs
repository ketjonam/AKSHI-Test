...
[Category("MIE")]
[Category("11140")]
public class _11140_NIPTWEB : BiznesTestBase
{
    protected override string ServiceCode => "11140";
    protected override string? ServiceTitle => "_11140_NIPT_Aplikim_i_Ri";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    private const string ExpectedServiceName =
        "Aplikim për ndryshim ose shtesa të drejtuesit/drejtuesve teknikë të shoqërisë në fushën e studimit e projektimit dhe/ose mbikëqyrjes e kolaudimit";
    private const string DocumentPath = @"C:\Users\Kreatx\Downloads\Signed_TEST_signed.pdf";
    private const string TechnicalDirectorNid = "J55728107R";
    private const int TotalSteps = 4;

    private static readonly (string Id, string Title)[] ModalDocuments =
    {
        ("fuDiplomaUpload", "Diploma"),
        ("fuCVUpload", "CV e individit, ku pasqyrohet veprimtaria profesionale e tij, e kryer nga marrja e diplomës"),
        ("fuDokJustifikuesUpload", "Dokumentacion teknik justifikues për veprimtarinë e kryer"),
        ("fuVetdeklarimUpload", "Vetëdeklarim i individit (sipas formatit 6/1"),
    };

    private static readonly (string Id, string Title, bool Required)[] Step4Documents =
    {
        ("fuSubVetdeklarimUpload",
            "Vetëdeklarim i përfaqësuesit/përfaqësuesve ligjorë (sipas formatit të lidhjes nr.6/1)", true),
        ("fuSubKontrataNdermjetUpload",
            "Kontratat midis përfaqësuesit ligjor dhe drejtuesve teknike, me afat të përcaktuar (mbi 1 vit)", true),
        ("fuSubVetdekDTUpload",
            "Vetëdeklarim i drejtuesit/drejtuesve teknikë (sipas formatit të lidhjes nr.6/5)", true),
        ("fuSubListaNotaveUpload",
            "Lista e notave të shkollës së lartë (për drejtuesit teknikë që kanë mesatare të notave mbi 9 dhe kanë vetëm 3 vite eksperiencë)", false),
        ("fuSubMandatPagesaUpload", "Pagesa e tarifës/tarifave", false),
    };
    ...
    public void _11140_NIPT_Aplikim_i_Ri()
    {
        ...
        AssertDisabledById("legalRepresentative", "Migen  Luan  Dërstila |");
        ...
        IWebElement adresa2 = wait.Until(ExpectedConditions.ElementExists(By.Id("additionalAddress")));
        ...
        IWebElement nrTel = wait.Until(ExpectedConditions.ElementExists(By.Id("contactPhone")));
        ...
        IWebElement nrCel = wait.Until(ExpectedConditions.ElementExists(By.Id("contactMobile")));
        ...
        IWebElement email = wait.Until(ExpectedConditions.ElementExists(By.Id("contactEmail")));
        ...
        IWebElement table = wait.Until(ExpectedConditions.ElementIsVisible(
            By.CssSelector("table.MuiTable-root, table.table-striped, table")));
        ...
        Assert.That(FindStep3RowLabel("Nr. i licencës individuale").Displayed, Is.True);
        Assert.That(FindStep3RowLabel("NID").Displayed, Is.True);
        Assert.That(FindStep3RowLabel("Shto drejtues teknik dhe apliko për shtesa").Displayed, Is.True);

        IWebElement licenseInput = FindStep3RowInput("Nr. i licencës individuale");
        Assert.That(InputValue(licenseInput), Is.EqualTo(string.Empty));
        Assert.That(FindStep3RowButton("Nr. i licencës individuale").Text.Trim(), Is.EqualTo("Shto"));

        IWebElement nidInput = FindStep3RowInput("NID");
        Assert.That(InputValue(nidInput), Is.EqualTo(string.Empty));
        IWebElement nidShtoBtn = FindStep3RowButton("NID");
        Assert.That(nidShtoBtn.Text.Trim(), Is.EqualTo("Shto"));

        IWebElement shtesaInput = FindStep3RowInput("Shto drejtues teknik dhe apliko për shtesa");
        Assert.That(InputValue(shtesaInput), Is.EqualTo(string.Empty));
        Assert.That(FindStep3RowButton("Shto drejtues teknik dhe apliko për shtesa").Text.Trim(),
            Is.EqualTo("Shto"));
        ...
        SafeClick(By.CssSelector("button.ealb-btn-continue"));
        AssertDrejtuesRequiredMessage();

        Log("Plotëso NID dhe kliko Shto");
        FillInput(nidInput, TechnicalDirectorNid);
        SafeClick(By.XPath(
            "//label[contains(.,'NID')]/ancestor::div[contains(@class,'row')][1]//button[contains(.,'Shto')]"));

        Log("Assert modal Shtoni drejtues teknik");
        ...
        IWebElement shtoModalBtn = GetModalFooterButton(modal, "Shto");
        Assert.That(anuloBtn, Is.Not.Null, "Butoni Anulo nuk u gjet.");
        Assert.That(shtoModalBtn, Is.Not.Null, "Butoni Shto nuk u gjet.");

        Log("Assert seksioni Detajet e individit");
        Assert.That(modal.FindElement(
            By.XPath(".//h5[contains(.,'Detajet e individit')]")).Displayed, Is.True);
        AssertDisabledModalInput(modal, "NID", TechnicalDirectorNid);
        AssertDisabledModalInput(modal, "Emri", "Ketjona");
        AssertDisabledModalInput(modal, "Mbiemri", "Mema");
        AssertDisabledModalInput(modal, "Vendlindja", "Kavajë");
        AssertDisabledModalInput(modal, "Qarku", "KAVAJË");
        AssertDisabledModalInput(modal, "Atësia", "Mersin");
        AssertDisabledModalInput(modal, "Amësia", "Aishe");
        AssertDisabledModalInput(modal, "Rrethi", "TIRANË");
        AssertDisabledModalInput(modal, "Statusi civil", "SIN");
        AssertDisabledModalInput(modal, "Emri NJQV", "Shkolla \"3 Deshmoret\"");
        AssertDisabledModalInput(modal, "Rruga",
            "THABIT REXHA 04040156; Nd. 6; H. 2; ; KAVAJË; KAVAJË; 2501; KAVAJË");
        AssertDisabledModalInput(modal, "Datëlindja", "28.07.1995");
        ...
        AssertEditableModalInput(modal, "Nr. i tel.", string.Empty);
        AssertEditableModalInput(modal, "Nr. i cel.", string.Empty);
        AssertEditableModalInput(modal, "Email", string.Empty);
        ...
        Assert.That(
            modal.FindElements(By.XPath(
                    ".//*[contains(normalize-space(),'Vetëdeklarim i individit (sipas formatit 6/1 në rastin e aplikimit për studimin e projektimin)')]"))
                .Any(x => x.Displayed),
            Is.True);
        Assert.That(
            modal.FindElements(By.XPath(
                    ".//*[contains(normalize-space(),'Vetëdeklarim i individit (sipas formatit 6/2 në rastin e aplikimit për mbikëqyrje e kolaudim)')]"))
                .Any(x => x.Displayed),
            Is.True);
        ...
        Log("Kliko Shto ne modal");
        SafeClick(By.XPath(
            "//div[contains(@class,'custom-modal-footer')]//button[.//b[contains(normalize-space(),'Shto')] or contains(normalize-space(),'Shto')]"));
        ...
        bool step3ValidationStillVisible = driver.FindElements(By.XPath(
                "//*[contains(normalize-space(),'të paktën një drejtues teknik')]"))
            .Any(x =>
            {
        ...
        Assert.That(step3ValidationStillVisible, Is.False,
            "Pas Shto, drejtuesi teknik nuk u ruajt sepse validimi i Step 3 u shfaq përsëri.");
        ...
        Assert.That(
            driver.FindElements(By.XPath("//a[contains(.,'Shkarko')]")).Count,
            Is.GreaterThanOrEqualTo(2));
    ...
    private IWebElement FindStep3RowLabel(string labelPart)
    {
        return wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath($"//label[contains(.,'{labelPart}')]")));
    }

    private IWebElement FindStep3RowInput(string labelPart)
    {
        return wait.Until(ExpectedConditions.ElementExists(
            By.XPath($"//label[contains(.,'{labelPart}')]/ancestor::div[contains(@class,'row')][1]//input[contains(@class,'ealb-input')]")));
    }

    private IWebElement FindStep3RowButton(string labelPart)
    {
        return wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath($"//label[contains(.,'{labelPart}')]/ancestor::div[contains(@class,'row')][1]//button[contains(.,'Shto')]")));
    }

    private void AssertDrejtuesRequiredMessage()
    {
        IWebElement drejtuesError = wait.Until(d =>
        {
            var inline = d.FindElements(By.XPath(
                    "//*[contains(normalize-space(),'të paktën një drejtues teknik')]"))
                .FirstOrDefault(e =>
                {
            ...
            var alert = d.FindElements(By.CssSelector(".alert-modal-container"))
                .FirstOrDefault(e =>
                {
                    try { return e.Displayed; }
                    catch { return false; }
        ...
        Assert.That(drejtuesError.Text, Does.Contain("drejtues teknik"));

        var closeBtn = driver.FindElements(By.CssSelector(".alert-modal-button--primary"))
            .FirstOrDefault(e =>
            {
                try { return e.Displayed; }
                catch { return false; }
        ...
        wait.Until(d =>
            !d.FindElements(By.CssSelector(".alert-modal-container")).Any(x =>
            {
                try { return x.Displayed; }
                catch { return false; }
            }));
    ...
    private void AssertDisabledModalInput(IWebElement modal, string labelText, string expectedValue)
    {
        ...
        Assert.That(
            !input.Enabled || input.GetAttribute("disabled") != null || input.GetAttribute("readonly") != null,
            Is.True,
            $"Input '{labelText}' duhet të jetë disabled.");
    }
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle, bool required)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-11140"));
