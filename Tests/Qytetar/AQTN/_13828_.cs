...
[Category("AQTN")]
[Category("13828")]
public class _13828_ : QytetarNidJ557TestBase
{
    protected override string ServiceCode => "13828";
    protected override string? ServiceTitle => "PlaniRregulluesFshati";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    private const string ExpectedServiceName = "Dhënie e një plani rregullues qyteti (fshati)";
    ...
    public void PlaniRregulluesFshati()
    {
        ...
        AssertLockedInput("nrTelCel", "+355697008820");
        AssertLockedInput("datelindja", "13.04.1993");
        AssertLockedInput("vendlindja", "Korçë");
        AssertLockedInput("rrethi", "TIRANË");

        Log("Assert gjinia");
        IWebElement genderMale = wait.Until(ExpectedConditions.ElementExists(
            By.XPath("//label[normalize-space()='Mashkull']/preceding-sibling::input")));
        IWebElement genderFemale = wait.Until(ExpectedConditions.ElementExists(
            By.XPath("//label[normalize-space()='Femër']/preceding-sibling::input")));
        ...
        IWebElement formatiHartes = FindInput("formatiHartes");
        Assert.That(formatiHartes.GetAttribute("placeholder"), Is.EqualTo("ha"));
        Assert.That(formatiHartes.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(LabelForName("formatiHartes").Text, Does.Contain("*"));

        IWebElement vitiHartes = FindInput("vitiHartes");
        ...
        Assert.That(LabelForName("vitiHartes").Text, Does.Contain("*"));

        IWebElement specifikimi = FindInput("specifikimi");
        ...
        Assert.That(LabelForName("specifikimi").Text, Does.Contain("*"));

        IWebElement qytetiHartes = FindInput("qytetiHartes");
        ...
        Assert.That(LabelForName("qytetiHartes").Text, Does.Contain("*"));

        IWebElement rugaZona = FindInput("rugaZona");
        ...
        Assert.That(LabelForName("rugaZona").Text, Does.Contain("*"));

        IWebElement teTjera = FindInput("teTjera");
        ...
        IWebElement emertimiHartes = FindInput("emertimiHartes");
    ...
    {
        return driver.FindElement(By.XPath($"//*[@name='{name}']/preceding-sibling::label"));
    }
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-13828"));
