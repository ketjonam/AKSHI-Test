...
[Category("AQTN")]
[Category("13834")]
public class _13834_ : QytetarNidJ557TestBase
{
    protected override string ServiceCode => "13834";
    protected override string? ServiceTitle => "FotografiDixhitaleDokumentitTeknik";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    private const string ExpectedServiceName =
        "Dhënie fotografi dixhitale të dokumentit teknik";
    ...
    public void FotografiDixhitaleDokumentitTeknik()
    {
        ...
        Assert.That(driver.FindElement(By.CssSelector("label[for='emertimiDokumentit']")).Text,
            Does.Contain("*"));

        IWebElement vitiFotografise = wait.Until(ExpectedConditions.ElementIsVisible(
            By.Id("vitiFotografise")));
        Assert.That(vitiFotografise.GetAttribute("type"), Is.EqualTo("number"));
        Assert.That(vitiFotografise.GetAttribute("min"), Is.EqualTo("1"));
        Assert.That(vitiFotografise.GetAttribute("max"), Is.EqualTo("9999"));
        Assert.That(vitiFotografise.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(driver.FindElement(By.CssSelector("label[for='vitiFotografise']")).Text,
            Does.Not.Contain("*"));

        IWebElement autoriFotografise = wait.Until(ExpectedConditions.ElementIsVisible(
            By.Id("autoriFotografise")));
        Assert.That(autoriFotografise.GetAttribute("maxlength"), Is.EqualTo("120"));
        Assert.That(autoriFotografise.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));

        IWebElement nrFotografive = wait.Until(ExpectedConditions.ElementIsVisible(
            By.Id("nrFotografive")));
        Assert.That(nrFotografive.GetAttribute("type"), Is.EqualTo("number"));
        Assert.That(nrFotografive.GetAttribute("min"), Is.EqualTo("1"));
        Assert.That(nrFotografive.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));

        IWebElement teTjeraDokumentit = wait.Until(ExpectedConditions.ElementIsVisible(
            By.Id("teTjeraDokumentit")));
        Assert.That(teTjeraDokumentit.GetAttribute("maxlength"), Is.EqualTo("120"));
        Assert.That(teTjeraDokumentit.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(driver.FindElement(By.CssSelector("label[for='teTjeraDokumentit']")).Text,
            Does.Not.Contain("*"));

        Log("Assert specifikimi i dokumentit arkivor");
        IWebElement specifikimiGjateProjektimit = wait.Until(ExpectedConditions.ElementExists(
            By.Id("specifikimiGjateProjektimit")));
        IWebElement specifikimiGjateNdertimit = wait.Until(ExpectedConditions.ElementExists(
            By.Id("specifikimiGjateNdertimit")));
        IWebElement specifikimiMbasNdertimit = wait.Until(ExpectedConditions.ElementExists(
            By.Id("specifikimiMbasNdertimit")));
        Assert.That(specifikimiGjateProjektimit.GetAttribute("type"), Is.EqualTo("checkbox"));
        Assert.That(specifikimiGjateProjektimit.GetAttribute("value"), Is.EqualTo("GjateProjektimit"));
        Assert.That(specifikimiGjateProjektimit.GetAttribute("name"), Is.EqualTo("specifikimiDokumentit"));
        Assert.That(driver.FindElement(By.CssSelector("label[for='specifikimiGjateProjektimit']")).Text.Trim(),
            Is.EqualTo("Gjatë projektimit"));
        Assert.That(specifikimiGjateNdertimit.GetAttribute("type"), Is.EqualTo("checkbox"));
        Assert.That(specifikimiGjateNdertimit.GetAttribute("value"), Is.EqualTo("GjateNdertimit"));
        Assert.That(specifikimiGjateNdertimit.GetAttribute("name"), Is.EqualTo("specifikimiDokumentit"));
        Assert.That(driver.FindElement(By.CssSelector("label[for='specifikimiGjateNdertimit']")).Text.Trim(),
            Is.EqualTo("Gjatë ndërtimit"));
        Assert.That(specifikimiMbasNdertimit.GetAttribute("type"), Is.EqualTo("checkbox"));
        Assert.That(specifikimiMbasNdertimit.GetAttribute("value"), Is.EqualTo("MbasNdertimit"));
        Assert.That(specifikimiMbasNdertimit.GetAttribute("name"), Is.EqualTo("specifikimiDokumentit"));
        Assert.That(driver.FindElement(By.CssSelector("label[for='specifikimiMbasNdertimit']")).Text.Trim(),
            Is.EqualTo("Mbas ndërtimit"));
        ...
        emertimiDokumentit = wait.Until(ExpectedConditions.ElementIsVisible(By.Id("emertimiDokumentit")));
        FillInput(emertimiDokumentit, "test");
        Assert.That(emertimiDokumentit.GetAttribute("value").Trim(), Is.EqualTo("test"));

        specifikimiGjateNdertimit = wait.Until(ExpectedConditions.ElementToBeClickable(
            By.Id("specifikimiGjateNdertimit")));
        specifikimiGjateNdertimit.Click();
        Assert.That(specifikimiGjateNdertimit.Selected, Is.True);
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-13834"));
