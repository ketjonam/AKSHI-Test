...
[Category("AQTN")]
[Category("13847")]
public class _13847_ : QytetarNidJ557TestBase
{
    protected override string ServiceCode => "13847";
    protected override string? ServiceTitle => "FleteProjektiA0";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    private const string ExpectedServiceName =
        "Dhënie e një flete projekti mbi dimension A0";
    ...
    public void FleteProjektiA0()
    {
        ...
        AssertLockedInput("telCel", "+355697008820");
        AssertLockedInput("datelindja", "13.04.1993");
        AssertLockedInput("vendlindja", "Korçë");
        AssertLockedInput("emRrethi", "TIRANË");

        Log("Assert gjinia");
        IWebElement genderMale = wait.Until(ExpectedConditions.ElementExists(By.Id("gjiniaM")));
        IWebElement genderFemale = wait.Until(ExpectedConditions.ElementExists(By.Id("gjiniaF")));
        Assert.That(genderMale.GetAttribute("type"), Is.EqualTo("radio"));
        Assert.That(genderMale.GetAttribute("value"), Is.EqualTo("Mashkull"));
        Assert.That(genderMale.GetAttribute("disabled"), Is.Not.Null);
        Assert.That(driver.FindElement(By.CssSelector("label[for='gjiniaM']")).Text.Trim(),
            Is.EqualTo("Mashkull"));
        Assert.That(genderFemale.GetAttribute("type"), Is.EqualTo("radio"));
        Assert.That(genderFemale.GetAttribute("value"), Is.EqualTo("Femër"));
        Assert.That(genderFemale.GetAttribute("disabled"), Is.Not.Null);
        Assert.That(driver.FindElement(By.CssSelector("label[for='gjiniaF']")).Text.Trim(),
            Is.EqualTo("Femër"));
        ...
        Assert.That(FindLabel("shtetesia").Text, Does.Contain("*"));
        ...
        IWebElement adresa = FindInput("rruga");
        ...
        Assert.That(FindLabel("emertimiDokumentit").Text, Does.Not.Contain("*"));

        IWebElement vitiStudimit = FindInput("vitiStudimit");
        Assert.That(vitiStudimit.GetAttribute("type"), Is.EqualTo("number"));
        Assert.That(vitiStudimit.GetAttribute("min"), Is.EqualTo("1"));
        Assert.That(vitiStudimit.GetAttribute("max"), Is.EqualTo("9999"));
        Assert.That(vitiStudimit.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(FindLabel("vitiStudimit").Text.Trim(), Is.EqualTo("Viti i projektit (yyyy)"));
        Assert.That(FindLabel("vitiStudimit").Text, Does.Not.Contain("*"));
        ...
        AssertSpecifikimCheckbox("cbSpArkitekture", "Arkitekturë");
        AssertSpecifikimCheckbox("cbSpKonstruksion", "Konstruksion");
        AssertSpecifikimCheckbox("cbSpHidrosanitare", "Hidrosanitare");
        AssertSpecifikimCheckbox("cbSpElektrike", "Elektrike");
        AssertSpecifikimCheckbox("cbSpTeTjera", "Të tjera");
        ...
        SafeClick(By.Id("cbSpArkitekture"));
        Assert.That(wait.Until(ExpectedConditions.ElementExists(By.Id("cbSpArkitekture"))).Selected, Is.True);
        ...
        AssertDocumentUpload("fileHarta-compat", "Vendndodhja në hartë");
        ...
        UploadDocument("fileHarta-compat", documentPath);
    ...
    private IWebElement FindInput(string nameOrId)
    {
        return wait.Until(d =>
        {
            var byId = d.FindElements(By.Id(nameOrId));
            if (byId.Count > 0)
                return byId[0];

            var byName = d.FindElements(By.CssSelector($"input[name='{nameOrId}']"));
            if (byName.Count > 0)
                return byName[0];

            var labels = d.FindElements(By.CssSelector($"label[for='{nameOrId}']"));
            if (labels.Count > 0)
            {
                var following = labels[0].FindElements(By.XPath("following::input[1]"));
    ...
    private IWebElement FindLabel(string nameOrId)
    {
        var byFor = driver.FindElements(By.CssSelector($"label[for='{nameOrId}']"));
        if (byFor.Count > 0)
            return byFor[0];

        return driver.FindElement(By.XPath($"//input[@name='{nameOrId}']/preceding-sibling::label"));
    }

    private void AssertLockedInput(string nameOrId, string expectedValue)
    {
        IWebElement input = FindInput(nameOrId);
        ...
        Assert.That(isLocked, Is.True, $"Fusha {nameOrId} duhet te jete readonly ose disabled");
    }
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-13847"));
