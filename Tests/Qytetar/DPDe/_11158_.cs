...
[Category("DPDe")]
[Category("11158")]
public class _11158_ : QytetarNidJ557TestBase
{
    ...
    public void CertifikateMinimumiAnijeMjetLundrues()
    {
        ...
        Assert.That(Step1Title.Text.Trim().ToUpperInvariant(),
            Is.EqualTo("PËRZGJIDH TIPIN E APLIKIMIT"));
        ...
        AssertRadioOption("renewal", "2", "Rinovim");
        ...
        Assert.That(driver.FindElement(By.Id("renewal")).Selected, Is.False);
        ...
        Assert.That(telFiks.GetAttribute("name"), Is.EqualTo("faxNo"));
        ...
        IWebElement Step4Title = WaitForStepTitle("EMRI I MJETIT");
        Assert.That(Step4Title.Text.Trim().ToUpperInvariant(),
            Is.EqualTo("EMRI I MJETIT"));
        ...
        IWebElement teDhenatMjetit = FindInputByName("vehicleType");
        Assert.That(teDhenatMjetit.GetAttribute("type"), Is.EqualTo("text"));
        Assert.That(teDhenatMjetit.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(teDhenatMjetit.GetAttribute("disabled"), Is.Null);
        ...
        IWebElement nrImo = FindInputByName("imoNumber");
        ...
        IWebElement materialNdertimi = FindInputByName("constructionMaterial");
        ...
        IWebElement vitiNdertimit = FindInputByName("constructionYear");
        Assert.That(vitiNdertimit.GetAttribute("type"), Is.EqualTo("text"));
        ...
        AssertRequiredError("Të dhënat e mjetit");
        ...
        FillInput(FindInputByName("vehicleType"), "Anija Test");
        FillInput(FindInputByName("length"), "15");
        FillInput(FindInputByName("imoNumber"), "IMO9876543");
        FillInput(FindInputByName("width"), "5");
        FillInput(FindInputByName("constructionMaterial"), "Çelik");
        FillInput(FindInputByName("grt"), "30");
        FillInput(FindInputByName("constructionYear"), "2015");
        FillInput(FindInputByName("nrt"), "18");

        Assert.That(FindInputByName("vehicleType").GetAttribute("value").Trim(), Is.EqualTo("Anija Test"));
        Assert.That(FindInputByName("length").GetAttribute("value").Trim(), Is.EqualTo("15"));
        Assert.That(FindInputByName("imoNumber").GetAttribute("value").Trim(), Is.EqualTo("IMO9876543"));
        Assert.That(FindInputByName("width").GetAttribute("value").Trim(), Is.EqualTo("5"));
        Assert.That(FindInputByName("constructionMaterial").GetAttribute("value").Trim(), Is.EqualTo("Çelik"));
        Assert.That(FindInputByName("grt").GetAttribute("value").Trim(), Is.EqualTo("30"));
        Assert.That(FindInputByName("constructionYear").GetAttribute("value").Trim(), Is.EqualTo("2015"));
        ...
        AssertDocumentUpload(
            "mandatePayment_docUpload",
            "Mandatpagesa + faturën për pajisjen me certifikatë minimumi për anijet / mjetet lundruese");

        Log("Assert linkun e shkarkimit te fatures");
        IWebElement fatureLink = wait.Until(ExpectedConditions.ElementIsVisible(
            By.CssSelector("a[href='/service/Documents_11158/11158_fature.pdf']")));
        Assert.That(fatureLink.Text.Trim(), Is.EqualTo("Shkarkoni këtu"));

        Log("Assert dokumentet e administrates");
        Assert.That(driver.FindElement(
            By.XPath("//*[contains(.,'Raport Inspektimi nga KSHF')]")).Displayed, Is.True);
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-11158"));
