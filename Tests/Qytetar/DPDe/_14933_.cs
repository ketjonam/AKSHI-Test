...
[Category("DPDe")]
[Category("14933")]
public class _14933_ : QytetarNidJ557TestBase
{
    protected override string ServiceCode => "14933";
    protected override string? ServiceTitle => "PasaporteDetareShtetasShqiptarTeHuaj";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    private const string ExpectedServiceName =
        "Aplikim për regjistrim dhe pajisje me pasaportë detare për shtetas shqiptarë, të huaj, pa shtetësi / Registration and equipment with maritime passport for albanian, foreign, stateless citizens";
    ...
    [Test]
    public void PasaporteDetareShtetasShqiptarTeHuaj()
    {
        ...
        AssertRadioOption("nationalityForeign", "nationality", "Shtetas i huaj/Pa shtetësi");
        ...
        IWebElement applicationType1 = wait.Until(ExpectedConditions.ElementExists(By.Id("applicationType1")));
        Assert.That(applicationType1.GetAttribute("type"), Is.EqualTo("radio"));
        Assert.That(applicationType1.GetAttribute("name"), Is.EqualTo("applicationType"));
        Assert.That(applicationType1.GetAttribute("value"), Is.EqualTo("Ndryshim të dhënash personale"));
        Assert.That(applicationType1.Selected, Is.True);
        Assert.That(driver.FindElement(By.CssSelector("label[for='applicationType1']")).Text.Trim(),
            Is.EqualTo("Ndryshim të dhënash personale"));

        AssertRadioOption("applicationType2", "applicationType", "Humbje");
        Assert.That(driver.FindElement(By.Id("applicationType2")).GetAttribute("value"),
            Is.EqualTo("Humbje"));
        AssertRadioOption("applicationType3", "applicationType", "Dëmtim");
        Assert.That(driver.FindElement(By.Id("applicationType3")).GetAttribute("value"),
            Is.EqualTo("Dëmtim"));
        AssertRadioOption("applicationType4", "applicationType", "Përfundim rubrikave(Imbarkim/Zbarkim)");
        Assert.That(driver.FindElement(By.Id("applicationType4")).GetAttribute("value"),
            Is.EqualTo("Përfundim rubrikave(Imbarkim/Zbarkim)"));
        ...
        Log("Ploteso Nr.Tel Fiks dhe Kodi Postar");
        ...
        IWebElement gjatesia = FindInputByLabel("Gjatësia(cm)");
        ...
        IWebElement shenjaTeVecanta = FindInputByLabel("Shenja të veçanta");
        ...
        IWebElement ngjyraSyve = FindInputByLabel("Ngjyra e syve");
        ...
        FillInput(FindInputByLabel("Gjatësia(cm)"), "165");
        FillInput(FindInputByLabel("Shenja të veçanta"), "Asnjë");
        FillInput(FindInputByLabel("Ngjyra e syve"), "Kafe");

        Assert.That(FindInputByLabel("Gjatësia(cm)").GetAttribute("value").Trim(), Is.EqualTo("165"));
        Assert.That(FindInputByLabel("Shenja të veçanta").GetAttribute("value").Trim(), Is.EqualTo("Asnjë"));
        Assert.That(FindInputByLabel("Ngjyra e syve").GetAttribute("value").Trim(), Is.EqualTo("Kafe"));
        ...
        IWebElement fatureLink = wait.Until(ExpectedConditions.ElementIsVisible(
            By.CssSelector("a[href='/service/Documents_14933/11159_mandatpagesa.pdf']")));
        ...
        Log("Assert document-upload Dokumenti ekzistues");
        AssertDocumentUpload(
            "dokEkzistuesUpload",
            "Dokumenti ekzistues");
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(@class,'fw-bold') and contains(.,'Dokumenti ekzistues')]//span[normalize-space()='*']")).Displayed, Is.True);
        ...
        UploadDocument("fotoUpload", DocumentPath);
        UploadDocument("dokEkzistuesUpload", DocumentPath);
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-14933"));
