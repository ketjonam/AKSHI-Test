...
public class _10092_ : QytetarNidJ257TestBase
{
    ...
    private const string ExpectedServiceName = "Aplikim për regjistrim ciklomotori";
    ...
    {
        ("AMEPJLTUpload", "Autorizim nga MEPJ për lëshimin e targës CD, TR, HCC"),
        ("AUTBLUpload", "Autorizim nga Policia vetëm kur mjeti është i blinduar"),
        ("DAVCUSTUpload", "Dokument zhdoganimi i mjetit DAV (kur mjeti është blerë jashtë vendit)"),
        ("DTOMPNUpload", "Dokumenti teknik i origjinës së mjetit i përkthyer dhe noterizuar"),
        ("DVFITPRUpload", "Dokumenti i fitimit të pronësisë (varet nga shteti ku është blerë)"),
        ("FKIUpload", "Fotokopje e kartës identifikuese/ shërbimit me vulë të njomë"),
        ("IDCARDUpload", "Dokument Identifikimi"),
        ("PPUpload", "Prokurë e posaçme"),
        ("RBSDOCUpload", "Konfirmimi nga RBS"),
        ("VEHSIGUpload", "Siguracion i mjetit"),
    };
    ...
    public void Regjistrim_Ciklomotori()
    {
        ...
        AssertDisabledByName("telNo", "+355684053531");
        ...
        Assert.That(driver.FindElement(
            ...
            .Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//li[contains(.,'Fotokopje Lejedrejtimi')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//li[contains(.,'Dokumenti i identifikimit të treguesve të mjetit (kontrolli fizik)')]"))
            .Displayed, Is.True);
        ...
        IWebElement consent = wait.Until(ExpectedConditions.ElementExists(By.Id("consentCheckbox")));
        Assert.That(consent.GetAttribute("type"), Is.EqualTo("checkbox"));
        Assert.That(driver.FindElement(By.CssSelector("label[for='consentCheckbox']")).Text,
        ...
        IWebElement consentCheck = driver.FindElement(By.Id("consentCheckbox"));
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-10092"));
