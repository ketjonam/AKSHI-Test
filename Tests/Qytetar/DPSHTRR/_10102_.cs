...
[Category("DPSHTRR")]
[Category("10102")]
public class _10102_ : QytetarNidJ257TestBase
{
    protected override string ServiceCode => "10102";
    protected override string? ServiceTitle => "Ndryshim_Gjeneralitetesh";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    private const string ExpectedServiceName = "Kërkesë për ndryshim gjeneralitetesh";
    ...
    public void Ndryshim_Gjeneralitetesh()
    {
        ...
        IWebElement Step1Title = WaitForStepTitle("TË DHËNA RAJONI");
        Assert.That(Step1Title.Text.Trim().ToUpperInvariant(),
            Does.StartWith("TË DHËNA RAJONI"));
        ...
        AssertDisabledApplicantField("birthPlace", "Gjinar, Elbasan");
        AssertDisabledApplicantField("email", "migen.derstila@kreatx.com");
        AssertDisabledApplicantField("phoneNumber", "+355684053531");

        IWebElement adresa = FindApplicantField("address");
        ...
        IWebElement chassisNr = FindByName("chassisNr");
        Assert.That(chassisNr.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Nr. Shasisë").Text, Does.Contain("*"));

        IWebElement plate = FindByName("plate");
        Assert.That(plate.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        ...
        FillInput(chassisNr, "WAUZZZ4G4EN070522");
        FillInput(plate, "AB770PP");
        ...
        Assert.That(driver.FindElement(
            By.XPath("//h5[contains(.,'Dokumente që ngarkohen nga aplikanti')] | //h6[contains(.,'Dokumente që ngarkohen nga aplikanti')]"))
            .Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//h5[contains(.,'Dokumente që sigurohen nga nëpunësit e administratës')] | //h6[contains(.,'Dokumente që sigurohen nga nëpunësit e administratës')]"))
            .Displayed, Is.True);

        Log("Assert document-upload");
        var uploadHosts = wait.Until(d =>
        {
            var hosts = d.FindElements(By.TagName("document-upload"));
            return hosts.Count > 0 ? hosts : null;
        });
        Assert.That(uploadHosts.Count, Is.GreaterThan(0));
        foreach (var docUpload in uploadHosts)
            AssertDocumentUploadHost(docUpload);
        ...
        var uploadIds = driver.FindElements(By.TagName("document-upload"))
            .Select(h => h.GetAttribute("id"))
            .Where(id => !string.IsNullOrWhiteSpace(id))
            .ToList();
        Assert.That(uploadIds.Count, Is.GreaterThan(0), "Nuk u gjet asnjë document-upload me id");
        foreach (var id in uploadIds)
            UploadDocument(id!, DocumentPath);
    ...
    private void AssertDocumentUploadHost(IWebElement docUpload)
    {
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-10102"));
        ...
        Assert.That(docUpload.GetAttribute("button-label"), Does.Contain("Kliko për të ngarkuar dokument"));
        ...
        Assert.That(shadow.FindElement(By.CssSelector("[data-role='dropzone-text']")).Text.Trim(),
            Does.Contain("Kliko për të ngarkuar dokument"));
