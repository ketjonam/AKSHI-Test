...
[Category("ISSH")]
[Category("6157")]
public class _6157_ : QytetarNidJ557TestBase
{
    protected override string ServiceCode => "6157";
    protected override string? ServiceTitle => "AplikimPensionInvaliditetiSuplementar";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    [Test]
    public void AplikimPensionInvaliditetiSuplementar()
    {
        OpenNewApplicationFromServicePage("Aplikim për pension invaliditeti (KMCAP)");

        Log("Assert Title");
        IWebElement Step1Title = wait.Until(ExpectedConditions.ElementIsVisible(
            By.CssSelector("h4.text-uppercase")));
        Assert.That(Step1Title.Text.Trim().ToUpperInvariant(),
            Is.EqualTo("KËRKESË PËR CAKTIM PENSIONI INVALIDITETI, AKSIDENT NË PUNË, SËMUNDJE PROFESIONALE"));
        ...
        IWebElement nrInput = wait.Until(ExpectedConditions.ElementIsVisible(By.Id("registrationNumber")));
        ...
        IWebElement dateInput = wait.Until(ExpectedConditions.ElementIsVisible(By.Id("registrationDate")));
        string expectedDate = DateTime.Now.ToString("d/M/yyyy");
        Assert.That(dateInput.GetAttribute("value").Trim(), Is.EqualTo(expectedDate));
        Assert.That(dateInput.GetAttribute("readonly"), Is.Not.Null);

        Log("Assert DRSSH ka opsionet e drejtorive");
        IWebElement drsshSelect = wait.Until(ExpectedConditions.ElementExists(By.Id("drsshSelect")));
        ...
        IWebElement alsshSelect = wait.Until(ExpectedConditions.ElementExists(By.Id("agencySelect")));
        ...
        IWebElement alsshError = wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath("//form//label[contains(.,'ALSSH')]/following::div[contains(@class,'text-danger')][1]")));
        Assert.That(alsshError.Text.Trim(), Is.EqualTo("Përzgjidhni një vlerë për të vazhduar"));

        Log("Zgjidh Drejtoria Tirane");
        SelectDropdownByValue(wait.Until(ExpectedConditions.ElementExists(By.Id("drsshSelect"))), "11");
        ...
        wait.Until(d =>
        {
            try
            {
                var agency = d.FindElement(By.Id("agencySelect"));
        ...
        IWebElement alsshEnabled = driver.FindElement(By.Id("agencySelect"));
        var alsshOptions = new SelectElement(alsshEnabled);
        Assert.That(alsshEnabled.GetAttribute("readonly"), Is.Null);
        ...
        {
            var titles = d.FindElements(By.XPath("//form//h5[contains(@class,'text-uppercase')]"));
            if (titles.Count == 0)
                return null;
        ...
        Log("Assert 6 hapa, dy hapat e pare aktiv");
        ...
        IWebElement rrethiVendlindja = FindSectionInput("Vendlindja", "Rrethi");
        ...
        Assert.That(rrethiVendlindja.GetAttribute("value"), Is.EqualTo(string.Empty));
        ...
        Assert.That(rrethiVendlindja.GetAttribute("maxlength"), Is.EqualTo("100"));
        ...
        IWebElement rrethiAdresa = FindSectionInput("Adresa", "Rrethi");
        ...
        Assert.That(rrethiAdresa.GetAttribute("value"), Is.EqualTo(string.Empty));
        ...
        Assert.That(rrethiAdresa.GetAttribute("maxlength"), Is.EqualTo("100"));

        Log("Assert fushat e kontaktit");
        IWebElement nrTel1 = FindSectionInput("Kontakt", "Nr.tel 1");
        IWebElement email = FindSectionInput("Kontakt", "Email");
        IWebElement nrTel2 = FindSectionInput("Kontakt", "Nr.tel 2");
        Assert.That(nrTel1.GetAttribute("value").Trim(), Is.EqualTo("0676041404"));
        Assert.That(nrTel1.GetAttribute("readonly"), Is.Not.Null);
        ...
        FillInput(FindSectionInput("Adresa", "Rrethi"), "Kavajë");

        Log("Ploteso Nr.tel 2");
        FillInput(FindSectionInput("Kontakt", "Nr.tel 2"), "0676041404");
        ...
        IWebElement Step3Title = wait.Until(d =>
        {
            ...
            return titles[0].Text.Trim().ToUpperInvariant()
                == "KËRKESA PËR PENSION INVALIDITETI, PENSION SUPLEMENTAR INVALIDITETI, SHTESA/KOMPENSIME DHE MËNYRA E PAGESËS"
                ? titles[0]
                : null;
        });
        Assert.That(Step3Title.Text.Trim().ToUpperInvariant(),
            Is.EqualTo("KËRKESA PËR PENSION INVALIDITETI, PENSION SUPLEMENTAR INVALIDITETI, SHTESA/KOMPENSIME DHE MËNYRA E PAGESËS"));
        ...
        Log("Assert 6 hapa, tre hapat e pare aktiv");
        steps = driver.FindElements(By.CssSelector(".ealb-step"));
        Assert.That(steps.Count, Is.EqualTo(6));
        Assert.That(steps[0].GetAttribute("class"), Does.Contain("active"));
        Assert.That(steps[1].GetAttribute("class"), Does.Contain("active"));
        ...
        Assert.That(driver.FindElement(
            By.XPath("//h5[contains(.,'Parashtroj kërkesën për përfitim të pensionit')]")).Displayed, Is.True);
        ...
        Log("Assert shtesat mbi pension jane te pazgjedhura");
        IWebElement shtesa38 = wait.Until(ExpectedConditions.ElementExists(
            By.XPath("//span[contains(.,'nenit 38')]/following::input[@type='checkbox'][1]")));
        IWebElement shtesa39 = wait.Until(ExpectedConditions.ElementExists(
            By.XPath("//span[contains(.,'nenit 39')]/following::input[@type='checkbox'][1]")));
        Assert.That(shtesa38.Selected, Is.False);
        Assert.That(shtesa39.Selected, Is.False);
        ...
        IWebElement kompensimeParent = wait.Until(ExpectedConditions.ElementExists(
            By.XPath("//span[contains(.,'kompensimet si më poshtë')]/following::input[@type='checkbox'][1]")));
        ...
        IWebElement teArdhuraTeTjera = FindRowCheckbox("të ardhurat e tjera");
        ...
        IWebElement postalRadio = wait.Until(ExpectedConditions.ElementExists(By.Id("postalOption")));
        IWebElement bankRadio = wait.Until(ExpectedConditions.ElementExists(By.Id("bankOption")));
        Assert.That(postalRadio.Selected, Is.False);
        Assert.That(bankRadio.Selected, Is.False);
        Assert.That(FindPaymentSelect("postalOption").GetAttribute("disabled"), Is.Not.Null);
        Assert.That(FindPaymentSelect("bankOption").GetAttribute("disabled"), Is.Not.Null);
        ...
        Log("Zgjidh Pension invaliditeti nga semundje te pergjithshme dhe Suplementar shteteror invaliditeti");
        ClickRowCheckbox("sëmundje të përgjithshme");
        ClickRowCheckbox("8097");
        Assert.That(FindRowCheckbox("sëmundje të përgjithshme").Selected, Is.True);
        Assert.That(FindRowCheckbox("8097").Selected, Is.True);

        Log("Zgjidh terheqjen ne Qendren Paguese");
        SelectRadioById("postalOption");
        ...
        wait.Until(d =>
        {
            try
            {
                var postSelect = d.FindElement(
                    By.XPath("//input[@id='postalOption']/ancestor::div[contains(@class,'row')][1]//select"));
        ...
        IWebElement postalSelect = FindPaymentSelect("postalOption");
        ...
        IWebElement Step4Title = wait.Until(d =>
        {
            ...
            if (titles.Count == 0)
                return null;
            return titles[0].Text.Trim().ToUpperInvariant()
                == "DEKLARATA E PERIUDHAVE TË SIGURIMIT, MARRËDHËNIEVE FINANCIARE DHE PËRFITIMEVE TË TJERA"
                ? titles[0]
                : null;
        });
        Assert.That(Step4Title.Text.Trim().ToUpperInvariant(),
            Is.EqualTo("DEKLARATA E PERIUDHAVE TË SIGURIMIT, MARRËDHËNIEVE FINANCIARE DHE PËRFITIMEVE TË TJERA"));
        ...
        Log("Assert 6 hapa, kater hapat e pare aktiv");
        steps = driver.FindElements(By.CssSelector(".ealb-step"));
        Assert.That(steps.Count, Is.EqualTo(6));
        Assert.That(steps[0].GetAttribute("class"), Does.Contain("active"));
        Assert.That(steps[1].GetAttribute("class"), Does.Contain("active"));
        ...
        Log("Assert seksionet A, B.1, B.2, B.3, C, D, E");
        Assert.That(driver.FindElement(
            By.XPath("//h6[contains(.,'A. Deklaratë për ndryshimet e vendbanimit')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
        ...
        Assert.That(driver.FindElement(By.Id("chbKamDeklarimeJashte")).GetAttribute("type"), Is.EqualTo("checkbox"));
        Assert.That(driver.FindElement(By.Id("hasPensionBenefit")).GetAttribute("type"), Is.EqualTo("checkbox"));
        Assert.That(driver.FindElement(By.Id("hasOtherBenefits")).GetAttribute("type"), Is.EqualTo("checkbox"));
        ...
        IWebElement emptyPara1994 = wait.Until(ExpectedConditions.ElementExists(
            By.XPath("//h6[contains(.,'B.1.')]/following::table[1]//td[@colspan='7']")));
        ...
        Log("Assert checkbox-et B.2, B.3, D, E jane te pazgjedhura");
        IWebElement sectionB2 = wait.Until(ExpectedConditions.ElementExists(
            By.XPath("//span[contains(.,'ligjit nr. 7/2025')]/following::input[@type='checkbox'][1]")));
        Assert.That(sectionB2.Selected, Is.False);
        Assert.That(driver.FindElement(By.Id("chbKamDeklarimeJashte")).Selected, Is.False);
        Assert.That(driver.FindElement(By.Id("hasPensionBenefit")).Selected, Is.False);
        Assert.That(driver.FindElement(By.Id("hasOtherBenefits")).Selected, Is.False);
        ...
        IWebElement shenime = wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath("//label[contains(.,'Shënime të aplikantit')]/following-sibling::textarea")));
        ...
        FillInput(FindModalInput("Lloji i punës"), "Punonjës");
        Assert.That(FindModalInput("Lloji i punës").GetAttribute("maxlength"), Is.EqualTo("100"));
        ...
        FillInput(wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath("//span[contains(.,'shkëputur marrëdhëniet financiare me subjektin')]/following::input[@type='text'][1]"))),
        ...
        ClickMuiCheckbox("hasOtherBenefits");
        Assert.That(driver.FindElement(By.Id("hasOtherBenefits")).Selected, Is.True);

        Log("Kliko Vazhdo Step 4");
        SafeClick(By.CssSelector("button.ealb-btn-continue"));
        Thread.Sleep(3000);

        Log("Assert Step 5 dokumentacioni");
        IWebElement Step5Upload = wait.Until(ExpectedConditions.ElementExists(By.Id("6157-FileUpload1")));
        Assert.That(Step5Upload.Displayed, Is.True);
        ...
        Log("Assert 6 hapa, pese hapat e pare aktiv");
        steps = driver.FindElements(By.CssSelector(".ealb-step"));
        Assert.That(steps.Count, Is.EqualTo(6));
        Assert.That(steps[0].GetAttribute("class"), Does.Contain("active"));
        Assert.That(steps[1].GetAttribute("class"), Does.Contain("active"));
        ...
        AssertDocumentUpload("6157-FileUpload1", "Librezë Pune");

        Log("Assert document-upload Vertetim page 1994-2012");
        AssertDocumentUpload("6157-FileUpload2", "1994 deri në vitin 2012");

        Log("Assert document-upload Vendim gjykate");
        AssertDocumentUpload("6157-FileUpload3", "Vendim gjykate për njohje vjetërsie pune");

        Log("Assert document-upload Diploma");
        AssertDocumentUpload("6157-FileUpload4", "Diploma e shkollës së lartë");

        Log("Assert document-upload Vertetim per llojin e punes");
        AssertDocumentUpload("6157-FileUpload5", "Vërtetim për llojin e punës nga qendra e fundit e punës");

        Log("Assert document-upload Vertetim nga Prokuroria");
        AssertDocumentUpload("6157-FileUpload6", "Vërtetim nga Prokuroria");

        Log("Assert document-upload Flete Drejtimi per KMCAP");
        AssertDocumentUpload("6157-FileUpload7", "Fletë Drejtimi për KMCAP");

        Log("Assert document-upload Flete Dalje Spitali");
        AssertDocumentUpload("6157-FileUpload8", "Fletë Dalje Spitali");

        Log("Assert document-upload Ekzaminime te vecanta");
        AssertDocumentUpload("6157-FileUpload9", "Ekzaminime të veçanta");

        Log("Assert document-upload Analiza mjekesore");
        AssertDocumentUpload("6157-FileUpload10", "Analiza mjekësore");

        Log("Assert document-upload Proces Verbal i Aksidentit");
        AssertDocumentUpload("6157-FileUpload11", "Proces Verbal i Aksidentit në Punë");

        Log("Assert document-upload Te tjera");
        AssertDocumentUpload("6157-fuOthers", "Të tjera");
        ...
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'që nga viti 2012 deri në momentin e lënies së punës')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'Shërbimit të Detyrueshëm Ushtarak')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'Certifikatë Martese')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'pagës së papunësisë')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'dënimit për motive politike')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'Dëshmi penaliteti')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'Pagesën kalimtare')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'pagesën e energjisë elektrike')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'ndryshimit të gjeneraliteteve')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'pagesën e paaftësisë së përkohshme')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'Certifikatë Familjare')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'Aktverifikime të periudhave të sigurimit')]")).Displayed, Is.True);

        Log("Assert dokumenta shtese per pension suplementar shteteror invaliditeti");
        Assert.That(driver.FindElement(
            By.XPath("//p[contains(.,'aplikon edhe për pension suplementar')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//h5[contains(.,'Dokumenta shtesë për pension suplementar shtetëror invaliditeti')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//h6[contains(.,'Dokumente që ngarkohen nga aplikanti')]")).Displayed, Is.True);
        AssertDocumentUpload("6157-FileUpload13", "Fotokopje e Librezës së punës");
        AssertDocumentUpload("6157-fuOthersSupl", "Të tjera");
        Assert.That(driver.FindElement(
            By.XPath("//h6[contains(.,'Dokumenta që sigurohen nga nëpunësit e administratës publike')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'vjetërsisë në shërbim/detyrë/funksion')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(
            By.XPath("//div[contains(.,'pagës referuese të detyrës/funksionit')]")).Displayed, Is.True);
        ...
        IWebElement kerkuesiName = wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath("//div[normalize-space()='Kërkuesi']/following::div[contains(@class,'border-bottom')][1]")));
        Assert.That(kerkuesiName.Text.Trim(), Is.EqualTo("Ketjona Mema"));
        Assert.That(kerkuesiName.GetAttribute("readonly"), Is.Not.Null);
        Assert.That(driver.FindElement(
        ...
        IWebElement officerText = wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath("//form//p[contains(.,'8097 / 1996')]")));
        Assert.That(officerText.Text.Trim(),
            Does.Contain("Pasi vlerësova kushtet ligjore"));
        Assert.That(officerText.Text.Trim(),
            Does.Contain("Ligjin nr. 7703, datë 11.05.1993"));
        Assert.That(officerText.Text.Trim(),
            Does.Contain("ligjit nr. 8097 / 1996"));
        Assert.That(officerText.Text.Trim(),
        ...
        //Assert.That(referenceNumber.Text, Does.Contain("6157-"));
    ...
    private IWebElement FindBankAccountInput()
    {
        ...
        return wait.Until(ExpectedConditions.ElementExists(
            By.XPath("//input[@id='bankOption']/ancestor::div[contains(@class,'row')][1]//input[@type='text']")));
    }
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-6157"));
        ...
        Assert.That(docUpload.GetAttribute("file-types"), Is.EqualTo(".doc,.docx,.txt,.pdf,.bmp,.gif,.png,.jpg,.jpeg"));
        ...
        Assert.That(shadow.FindElement(By.CssSelector("[data-role='hint']")).Text.Trim(),
            Is.EqualTo("Formatet e lejuara: DOC, DOCX, TXT, PDF, BMP, GIF, PNG, JPG, JPEG. Madhesia maksimale: 5MB."));
    }
