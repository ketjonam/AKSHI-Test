...
[Category("ISSH")]
[Category("7171")]
public class _7171_ : QytetarNidF602TestBase
{
    protected override string ServiceCode => "7171";
    protected override string? ServiceTitle => "MasatPaterhequraLundrues";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    [Test]
    public void MasatPaterhequraLundrues()
    {
        ...
        Assert.That(serviceName.Text.Trim(),
            Is.EqualTo("Masat e patërhequra të trajtimit të veçantë financiar për efektivët lundrues në pension"),
        ...
        Assert.That(Step1Title.Text.Trim().ToUpperInvariant(),
            Is.EqualTo("PUBLIKIMI I PENSIONEVE"));

        Log("Assert mesazhi i bosh");
        IWebElement emptyMessage = wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath("//div[contains(@style,'text-align: center') and normalize-space()='Nuk gjendet pension suplementar i pa tërhequr!']")));
        Assert.That(emptyMessage.Displayed, Is.True, "Mesazhi nuk eshte visible");
        Assert.That(emptyMessage.Text.Trim(),
            Is.EqualTo("Nuk gjendet pension suplementar i pa tërhequr!"));
