...
[Category("MEPJ")]
[Category("9765")]
public class _9765_ : QytetarNidJ557TestBase
{
    protected override string ServiceCode => "9765";
    protected override string? ServiceTitle => "AplikimPerIDPassaporte";
    ...
    private const string DocumentTitle =
        "Dokument identifikimi (Pasaportë model i vjetër ose leje për drejtim automjeti model elektronik";
    ...
    public void AplikimPerIDPassaporte()
    {
        ...
        AssertDisabledById("nid", Settings.Qytetar.Username);
        AssertDisabledById("name", "Katerina");
        AssertDisabledById("surname", "Jançe");
        AssertDisabledById("gender-display", "Femër");
        AssertDisabledById("birthdate", "13.04.1993");
        AssertDisabledById("fatherName", "Foti");
        AssertDisabledById("motherName", "Manushaqe");

        IWebElement city = FindById("city");
        Assert.That(city.GetAttribute("value").Trim(), Is.EqualTo("Korçë"));
        Assert.That(city.GetAttribute("disabled"), Is.Not.Null);

        IWebElement type = FindById("type");
        Assert.That(new SelectElement(type).SelectedOption.GetAttribute("value"), Is.EqualTo(string.Empty));

        IWebElement state = FindById("state");
        Assert.That(new SelectElement(state).SelectedOption.GetAttribute("value"), Is.EqualTo(string.Empty));
        ...
        IWebElement aplikojPerFemije = wait.Until(ExpectedConditions.ElementExists(By.Id("aplikojperFemije")));
        Assert.That(aplikojPerFemije.GetAttribute("type"), Is.EqualTo("checkbox"));
        ...
        SelectByValue(type, "13");
        SelectByValue(state, "1");
        ...
        Assert.That(new SelectElement(FindById("country")).SelectedOption.GetAttribute("value"),
        ...
        Assert.That(FindByName("teTjera").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        ...
        SelectByValue(FindById("country"), "2");
        ...
        IWebElement phone = FindById("phone");
        ...
        IWebElement consulate = FindById("consulate");
        ...
        IWebElement Step4Title = WaitForStepTitle("REZERVONI ORARIN");
        Assert.That(Step4Title.Text.Trim().ToUpperInvariant(), Does.StartWith("REZERVONI ORARIN"));
        ...
        Log("Assert fushat e rezervimit");
        Assert.That(FindLabel("Data nga").Text, Does.Contain("*"));
        Assert.That(FindLabel("Data deri").Text, Does.Contain("*"));
        Assert.That(FindById("fromDate").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(FindById("toDate").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));

        IWebElement shfaqOraretBtn = wait.Until(ExpectedConditions.ElementIsVisible(
            By.XPath("//button[contains(normalize-space(.),'Shfaq oraret e lira')]")));
        Assert.That(shfaqOraretBtn.Displayed, Is.True);
        ...
        FillInput(FindById("fromDate"), "29.04.2026");
        FillInput(FindById("toDate"), "15.05.2026");

        Log("Kliko butonin Shfaq oraret e lira");
        SafeClick(By.XPath("//button[contains(normalize-space(.),'Shfaq oraret e lira')]"));
        Thread.Sleep(2000);

        Log("Zgjidh daten dhe oren");
        SelectByValue(FindById("selectDate"), "05.05.2026");
        Thread.Sleep(500);
        SelectByValue(FindById("selectTime"), "10:00:00 - 11:00:00");
        ...
        Assert.That(driver.FindElement(
            ...
            .Displayed, Is.True);
        Assert.That(driver.FindElement(By.XPath("//li[contains(.,'Certifikatë lindje')]")).Displayed, Is.True);
        Assert.That(driver.FindElement(By.XPath("//li[contains(.,'Certifikatë familjare')]")).Displayed, Is.True);

        Log("Assert document-upload Dokument identifikimi");
        AssertDocumentUpload(IdUploadId, DocumentTitle);

        Log("Assert checkbox dokumentacioni");
        IWebElement consentCheckbox = wait.Until(ExpectedConditions.ElementExists(By.Id("adminDocumentsConsent")));
        ...
        IWebElement consentLabel = wait.Until(ExpectedConditions.ElementIsVisible(
            By.CssSelector("label[for='adminDocumentsConsent']")));
        ...
        Log("Kliko Dergo pa ngarkuar dokumentin dhe pa checkbox");
        ...
        SafeClick(By.CssSelector("label[for='adminDocumentsConsent']"));
        Thread.Sleep(500);
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-9765"));
