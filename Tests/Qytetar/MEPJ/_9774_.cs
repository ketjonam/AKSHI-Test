...
[Category("MEPJ")]
[Category("9774")]
public class _9774_ : QytetarNidJ557TestBase
{
    protected override string ServiceCode => "9774";
    protected override string? ServiceTitle => "PerkthimDokumenti";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    private const string ExpectedServiceName = "Aplikim për vërtetim përkthim dokumenti";
    private const string DocumentPath = @"C:\Users\Kreatx\Downloads\Test Dokument.pdf.pdf";
    private const string IdentifikimiUploadId = "dokumentIdentifikimi_doc_pdUpload";
    private const string DokumentiOrigjinalUploadId = "dokumentiOrigjinal_pdUpload";
    ...
    public void PerkthimDokumenti()
    {
        ...
        IWebElement vendlindjaQyteti = FindInputByLabel("Vendlindja/Qyteti");
        Assert.That(vendlindjaQyteti.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Vendlindja/Qyteti").Text, Does.Contain("*"));

        IWebElement shtetesiaDyte = FindInputByLabel("Shtetësia e dytë");
        ...
        Assert.That(new SelectElement(FindById("shteti")).SelectedOption.GetAttribute("value"),
            Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Shteti").Text, Does.Contain("*"));
        Assert.That(FindById("rruga").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        ...
        Assert.That(FindById("kodiPostar").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Kodi postar").Text, Does.Contain("*"));
        Assert.That(FindById("rajoni").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(FindById("teDhena").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        ...
        SelectByValue(FindById("shteti"), "2");
        FillInput(FindById("rruga"), "test");
        FillInput(FindById("qyteti"), "test");
        FillInput(FindById("kodiPostar"), "1001");
        ...
        IWebElement telNo = FindById("telNo");
        Assert.That(telNo.GetAttribute("value").Trim(), Is.EqualTo("+355697008820"));
        Assert.That(telNo.GetAttribute("readonly"), Is.Not.Null);
        Assert.That(telNo.GetAttribute("disabled"), Is.Not.Null);
        Assert.That(FindLabel("Nr. tel").Text, Does.Contain("*"));

        IWebElement contactCountry = FindById("shteti");
        Assert.That(new SelectElement(contactCountry).SelectedOption.GetAttribute("value"),
            Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Shteti").Text, Does.Contain("*"));

        IWebElement consularOffice = FindById("zyraKonsullore");
        ...
        SelectFirstWhenEnabled(By.Id("zyraKonsullore"));
        ...
        IWebElement Step4Title = WaitForStepTitle("KËRKESA PËR PËRKTHIMIN E DOKUMENTIT");
        Assert.That(Step4Title.Text.Trim().ToUpperInvariant(),
            Does.StartWith("KËRKESA PËR PËRKTHIMIN E DOKUMENTIT"));
        ...
        Log("Assert fushat e perkthimit");
        IWebElement caseDetails = FindById("caseDetails");
        Assert.That(caseDetails.TagName.ToLowerInvariant(), Is.EqualTo("textarea"));
        Assert.That(caseDetails.GetAttribute("value").Trim(), Is.EqualTo(string.Empty));

        IWebElement gjuhaOrigjinale = FindSelectByLabel("Gjuha dokumentit origjinal");
        Assert.That(new SelectElement(gjuhaOrigjinale).SelectedOption.GetAttribute("value"),
            Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Gjuha dokumentit origjinal").Text, Does.Contain("*"));

        IWebElement gjuhaPerkthyer = FindSelectByLabel("Gjuha në të cilën është përkthyer");
        Assert.That(new SelectElement(gjuhaPerkthyer).SelectedOption.GetAttribute("value"),
            Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Gjuha në të cilën është përkthyer").Text, Does.Contain("*"));
        ...
        FillInput(FindById("caseDetails"), "test");
        SelectByValue(FindSelectByLabel("Gjuha dokumentit origjinal"), "2");
        SelectByValue(FindSelectByLabel("Gjuha në të cilën është përkthyer"), "3");
        ...
        AssertDocumentUpload(IdentifikimiUploadId,
            "Dokument identifikimi (Pasaportë ose kartë identiteti shqiptare ose dokument apo certifikatë lindje e huaj)",
            required: false);
        AssertDocumentUpload(DokumentiOrigjinalUploadId,
            "Dokumenti që do të përkthehet", required: true);
        ...
        UploadDocument(DokumentiOrigjinalUploadId, DocumentPath);
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle, bool required)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-9774"));
