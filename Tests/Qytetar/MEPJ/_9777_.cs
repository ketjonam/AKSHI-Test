...
[Category("MEPJ")]
[Category("9777")]
public class _9777_ : QytetarNidJ557TestBase
{
    protected override string ServiceCode => "9777";
    protected override string? ServiceTitle => "VertetimCertifikatLindje";
    protected override ServiceStartMode StartMode => ServiceStartMode.NewApplication;
    protected override bool StartServiceOnSetup => false;

    private const string ExpectedServiceName = "Aplikim për vërtetim certifikatë lindje";
    private const string DocumentPath = @"C:\Users\Kreatx\Downloads\Test Dokument.pdf.pdf";
    private const string IdentifikimiUploadId = "docIdUpload";
    ...
    public void VertetimCertifikatLindje()
    {
        ...
        IWebElement aplikuesi = FindSelectByLabel("Aplikuesi");
        var aplikuesiSelect = new SelectElement(aplikuesi);
        Assert.That(aplikuesiSelect.SelectedOption.GetAttribute("value"), Is.EqualTo(string.Empty));
        Assert.That(aplikuesiSelect.Options[1].Text.Trim(), Is.EqualTo("Vet personi"));
        Assert.That(aplikuesiSelect.Options[2].Text.Trim(), Is.EqualTo("Prindi (për fëmijën)"));
        Assert.That(aplikuesiSelect.Options[3].Text.Trim(), Is.EqualTo("Kujdestari (për fëmijen)"));
        Assert.That(FindLabel("Aplikuesi").Text, Does.Contain("*"));
        ...
        Assert.That(new SelectElement(FindSelectByLabel("Shteti")).SelectedOption.GetAttribute("value"),
            Is.EqualTo(string.Empty));
        ...
        Assert.That(FindInputByLabel("Rajoni/Ndarja administrative").GetAttribute("value").Trim(),
            Is.EqualTo(string.Empty));
        Assert.That(FindInputByLabel("Të dhëna të tjera për adresën").GetAttribute("value").Trim(),
            Is.EqualTo(string.Empty));
        ...
        IWebElement nrTel = FindByName("nrTel");
        ...
        IWebElement contactCountry = FindSelectByLabel("Shteti");
        ...
        SelectByText(FindSelectByLabel("Shteti"), "Emiratet e Bashkuara Arabe");
        SelectFirstWhenEnabled(By.XPath(
            "//form//label[contains(.,'Zyra konsullore')]/following-sibling::select"));
        ...
        Assert.That(FindInputByLabel("Emri").GetAttribute("value").Trim(), Is.EqualTo("Katerina"));
        Assert.That(FindLabel("Emri").Text, Does.Contain("*"));
        Assert.That(FindInputByLabel("Mbiemri").GetAttribute("value").Trim(), Is.EqualTo("Jançe"));
        ...
        Assert.That(FindInputByLabel("Amësia").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Amësia").Text, Does.Contain("*"));
        Assert.That(FindLabel("Datëlindja").Text, Does.Contain("*"));
        Assert.That(FindInputByLabel("Vendlindja/Qyteti").GetAttribute("value").Trim(),
            Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Vendlindja/Qyteti").Text, Does.Contain("*"));
        Assert.That(new SelectElement(FindSelectByLabel("Vendlindja/Shteti")).SelectedOption
            .GetAttribute("value"), Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Vendlindja/Shteti").Text, Does.Contain("*"));
        Assert.That(FindInputByLabel("Vendbanimi").GetAttribute("value").Trim(), Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Vendbanimi").Text, Does.Contain("*"));
        Assert.That(new SelectElement(FindSelectByLabel("Gjinia")).SelectedOption.GetAttribute("value"),
            Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Gjinia").Text, Does.Contain("*"));
        Assert.That(new SelectElement(FindSelectByLabel("Gjendja Civile")).SelectedOption
            .GetAttribute("value"), Is.EqualTo(string.Empty));
        Assert.That(FindLabel("Gjendja Civile").Text, Does.Contain("*"));
        Assert.That(FindInputByLabel("Shtetësia").GetAttribute("value").Trim(), Is.EqualTo("Shqiptare"));
        Assert.That(FindLabel("Shtetësia").Text, Does.Contain("*"));
        Assert.That(FindInputByLabel("Mbiemri para martese").GetAttribute("value").Trim(),
            Is.EqualTo(string.Empty));
        ...
        FillInput(FindInputByLabel("Atësia"), "test");
        FillInput(FindInputByLabel("Amësia"), "test");
        IWebElement datelindjaPersonit = FindDateInputByLabel("Datëlindja");
        string datelindjaValue = datelindjaPersonit.GetAttribute("value")?.Trim() ?? string.Empty;
        if (string.IsNullOrEmpty(datelindjaValue))
            FillDate(datelindjaPersonit, "13.04.1993", 1993, 4, 13);
        FillInput(FindInputByLabel("Vendlindja/Qyteti"), "test");
        SelectByValue(FindSelectByLabel("Vendlindja/Shteti"), "1");
        FillInput(FindInputByLabel("Vendbanimi"), "test");
        SelectByValue(FindSelectByLabel("Gjinia"), "F");
        SelectByValue(FindSelectByLabel("Gjendja Civile"), "1");
    ...
    private void SelectByText(IWebElement select, string text)
    {
        ((IJavaScriptExecutor)driver).ExecuteScript(
            "arguments[0].scrollIntoView({block:'center'});",
            select);
        Thread.Sleep(300);
        new SelectElement(select).SelectByText(text);
        Thread.Sleep(800);
    }
    ...
    private void AssertDocumentUpload(string uploadId, string documentTitle, bool required)
    {
        ...
        Assert.That(docUpload.GetAttribute("application-reference"), Is.EqualTo("docstreamv2-9777"));
